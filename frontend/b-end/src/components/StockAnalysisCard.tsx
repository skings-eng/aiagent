...
const StockAnalysisCard: React.FC<StockAnalysisCardProps> = ({ stockData, lineConfig }) => {
  ...
  const getTrendIcon = (score: string) => {
    if (!score) return <BarChart3 className="w-5 h-5 text-gray-500" />;
    ...
    <div className="bg-white rounded-lg shadow-lg border border-gray-200 p-6 max-w-4xl mx-auto">
      ...
      <div className="border-b border-gray-200 pb-4 mb-6">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-2xl font-bold text-gray-900">{safeGet(stockData, 'name', '未知股票')}</h2>
            <p className="text-lg text-gray-600">{safeGet(stockData, 'name_ja', '未知')}</p>
            <div className="flex items-center space-x-2 mt-1">
              <span className="text-sm bg-blue-100 text-blue-800 px-2 py-1 rounded">
                {safeGet(stockData, 'code', '未知代码')}
              </span>
              <span className="text-sm bg-gray-100 text-gray-700 px-2 py-1 rounded">
                {safeGet(stockData, 'industry', '未知行业')}
              </span>
            </div>
          </div>
          <div className="text-right">
            <div className="flex items-center space-x-2">
              ...
              <span className="text-3xl font-bold text-gray-900">
                ¥{(safeGet(stockData, 'price', 0) || safeGet(stockData, 'current_price', 0) || 0).toLocaleString()}
              </span>
            </div>
          </div>
        </div>
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        <div className="bg-gray-50 rounded-lg p-4">
          <div className="flex items-center justify-between">
            ...
            {getTrendIcon(safeGet(stockData, 'summary.scores.trend'))}
          </div>
          <div className={`text-lg font-bold ${getScoreColor(safeGet(stockData, 'summary.scores.trend'))}`}>
            {safeGet(stockData, 'summary.scores.trend', '暂无数据')}
          </div>
        </div>
        <div className="bg-gray-50 rounded-lg p-4">
          <div className="flex items-center justify-between">
          ...
          <div className={`text-lg font-bold ${getScoreColor(safeGet(stockData, 'summary.scores.valuation'))}`}>
            {safeGet(stockData, 'summary.scores.valuation', '暂无数据')}
          </div>
        </div>
        <div className="bg-gray-50 rounded-lg p-4">
          <div className="flex items-center justify-between">
          ...
          <div className={`text-lg font-bold ${getScoreColor(safeGet(stockData, 'summary.scores.risk'))}`}>
            {safeGet(stockData, 'summary.scores.risk', '暂无数据')}
          </div>
        </div>
      </div>
      ...
      <div className="bg-blue-50 rounded-lg p-4 mb-6">
        ...
        <p className="text-blue-800">{safeGet(stockData, 'summary.suggestion', '暂无投资建议')}</p>
      </div>
          ...
          <div className="space-y-3">
            <div className="bg-gray-50 rounded p-3">
              <div className="flex justify-between items-center mb-1">
                <span className="text-sm font-medium text-gray-600">趋势评分</span>
                <span className="text-sm font-bold text-gray-900">{safeGet(stockData, 'technical.trend_score', '暂无数据')}</span>
              </div>
              <p className="text-xs text-gray-700">{safeGet(stockData, 'technical.trend_reason', '暂无分析')}</p>
            </div>
            <div className="grid grid-cols-2 gap-2">
              <div className="bg-gray-50 rounded p-2">
                <span className="text-xs font-medium text-gray-600">RSI</span>
                <div className="text-sm font-bold text-gray-900">{safeGet(stockData, 'technical.rsi', '暂无数据')}</div>
              </div>
              <div className="bg-gray-50 rounded p-2">
                <span className="text-xs font-medium text-gray-600">MACD</span>
                <div className="text-sm font-bold text-gray-900">{safeGet(stockData, 'technical.macd', '暂无数据')}</div>
              </div>
            </div>
            {safeGet(stockData, 'technical.support_levels', []).length > 0 && (
              <div className="bg-green-50 rounded p-2">
                <span className="text-xs font-medium text-green-700">支撑位</span>
                <div className="text-sm text-green-800">
                  {safeGet(stockData, 'technical.support_levels', []).map((level: number) => `¥${level.toLocaleString()}`).join(', ')}
                </div>
              </div>
            )}
            {safeGet(stockData, 'technical.resistance_levels', []).length > 0 && (
              <div className="bg-red-50 rounded p-2">
                <span className="text-xs font-medium text-red-700">阻力位</span>
                <div className="text-sm text-red-800">
                  {safeGet(stockData, 'technical.resistance_levels', []).map((level: number) => `¥${level.toLocaleString()}`).join(', ')}
                </div>
              </div>
            )}
          </div>
          ...
          <div className="space-y-3">
            <div className="grid grid-cols-2 gap-2">
              <div className="bg-gray-50 rounded p-2">
                ...
                <div className="text-sm font-bold text-gray-900">{safeGet(stockData, 'fundamental.pe', '暂无数据')}</div>
              </div>
              <div className="bg-gray-50 rounded p-2">
                <span className="text-xs font-medium text-gray-600">PEG比率</span>
                <div className="text-sm font-bold text-gray-900">{safeGet(stockData, 'fundamental.peg', '暂无数据')}</div>
              </div>
              <div className="bg-gray-50 rounded p-2">
                <span className="text-xs font-medium text-gray-600">投资回报率</span>
                <div className="text-sm font-bold text-gray-900">{safeGet(stockData, 'fundamental.roic', '暂无数据')}</div>
              </div>
              <div className="bg-gray-50 rounded p-2">
                <span className="text-xs font-medium text-gray-600">营收增长率</span>
                <div className="text-sm font-bold text-gray-900">{safeGet(stockData, 'fundamental.revenue_growth', '暂无数据')}</div>
              </div>
            </div>
            <div className="bg-blue-50 rounded p-3">
              <span className="text-sm font-medium text-blue-700">估值区间</span>
              <div className="text-sm font-bold text-blue-900">{safeGet(stockData, 'fundamental.valuation', '暂无数据')}</div>
            </div>
            <div className="bg-gray-50 rounded p-3">
              <span className="text-sm font-medium text-gray-600">现金流趋势</span>
              <div className="text-sm text-gray-900">{safeGet(stockData, 'fundamental.fcf_trend', '暂无数据')}</div>
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="space-y-3">
          ...
          <div className="space-y-2">
            <div className="bg-gray-50 rounded p-3">
              ...
              <div className="text-sm font-bold text-gray-900">{safeGet(stockData, 'sentiment.institutional_holding', '暂无数据')}</div>
            </div>
            <div className="bg-gray-50 rounded p-3">
              <span className="text-sm font-medium text-gray-600">主力资金流向</span>
              <div className="text-sm font-bold text-gray-900">{safeGet(stockData, 'sentiment.main_capital_flow', '暂无数据')}</div>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-3">
          ...
          <div className="space-y-2">
            <div className="bg-orange-50 rounded p-3">
              ...
              <div className="text-sm font-bold text-orange-900">{safeGet(stockData, 'risk.volatility', '暂无数据')}</div>
            </div>
            {safeGet(stockData, 'risk.risk_events', []).length > 0 && (
              <div className="bg-red-50 rounded p-3">
                <span className="text-sm font-medium text-red-700">风险事件</span>
                <ul className="text-sm text-red-800 mt-1">
                  {safeGet(stockData, 'risk.risk_events', []).map((event: string, index: number) => (
      ...
      <div className="mt-6 pt-4 border-t border-gray-200">
        ...
        <div className="text-xs text-gray-500 space-y-1">
          <div>市盈率: {safeGet(stockData, 'data_source.pe', '未知')}</div>
          <div>RSI: {safeGet(stockData, 'data_source.rsi', '未知')}</div>
          <div>估值: {safeGet(stockData, 'data_source.valuation', '未知')}</div>
        </div>
      </div>
    </div>
  );
